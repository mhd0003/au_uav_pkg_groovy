...
bool au_uav_ros::PlaneCoordinator::handleNewUpdate(au_uav_ros::TelemetryUpdate update, au_uav_ros::Command *newCommand)
{
	...
	if(!avoidancePath.empty() && update.distanceToDestination > -1 && update.distanceToDestination < COLLISION_THRESHOLD*2)
	{
		// now check if planeDest has been updated
		if (distanceBetween(avoidancePath.front(), planeDest) < COLLISION_THRESHOLD*2)
	...
	else if(!normalPath.empty() && update.distanceToDestination > -1 && update.distanceToDestination < COLLISION_THRESHOLD*2)
	{
		
		// now check if planeDest has been updated
		if (distanceBetween(normalPath.front(), planeDest) < COLLISION_THRESHOLD*2){
